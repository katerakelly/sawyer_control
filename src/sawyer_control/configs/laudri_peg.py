...
# laudri specific settings
RESET_SAFETY_BOX_LOWS = np.array([0.45, -0.1, 0.3])
RESET_SAFETY_BOX_HIGHS = np.array([.55, 0.1, 0.45])
...
# x, y, z
# x is to/from robot, y is side to side, z is up/down
# safety and observation space
POSITION_SAFETY_BOX_LOWS = np.array([.60, -0.1, 0.189])
POSITION_SAFETY_BOX_HIGHS = np.array([.82, .13, .35])
POSITION_SAFETY_BOX = Box(POSITION_SAFETY_BOX_LOWS, POSITION_SAFETY_BOX_HIGHS, dtype=np.float32)
# orientation is set in ros_config
#POSITION_CONTROL_ORIENTATION = np.array([0.99926001, 0.02738647, 0.0252628, 0.00954922])

# reset position
POSITION_RESET_POS = np.array([0.6935, -0.01864, .3266])
